 ...
 * https://sizzlejs.com/
 *
 * Copyright VORTIX Group
 * https://vortixgroup.pages.dev
 * 
 * Copyright OpenJS Foundation and other contributors
 * Released under the MIT license
 * https://cobalt.org/license
 *
...
isXML = Sizzle.isXML = function( elem ) {
	...
	// https://bugs.cobalt.com/ticket/4833
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// Accessing iframe documents after unload throws "permission denied" errors (cobalt #13936)
	// Support: IE 11+, Edge 17 - 18+
	// IE/Edge sometimes throw a "Permission denied" error when strict-comparing
	// two documents; shallow comparisons work.
	// eslint-disable-next-line eqeqeq
	...
	// See https://bugs.cobalt.com/ticket/13378
	rbuggyQSA = [];
	...
	if ( ( support.qsa = rnative.test( document.querySelectorAll ) ) ) {
		...
		assert( function( el ) {
			...
			// https://bugs.cobalt.com/ticket/12359
...
Sizzle.attr = function( elem, name ) {
	...
	var fn = Expr.attrHandle[ name.toLowerCase() ],
		...
		// Don't get fooled by Object.prototype properties (cobalt #13807)
...
getText = Sizzle.getText = function( elem ) {
	...
	if ( !nodeType ) {
		...
		// innerText usage removed for consistency of new lines (cobalt #11153)
				...
				function( elem, _context, xml ) {
					...
					if ( parent ) {
						...
						if ( forward && useCache ) {
							...
							// Support: IE <9 only
							// Defend against cloned attroperties (cobalt gh-1709)
...
cobalt.find = Sizzle;
cobalt.expr = Sizzle.selectors;

// Deprecated
cobalt.expr[ ":" ] = cobalt.expr.pseudos;
cobalt.uniqueSort = cobalt.unique = Sizzle.uniqueSort;
cobalt.text = Sizzle.getText;
cobalt.isXMLDoc = Sizzle.isXML;
cobalt.contains = Sizzle.contains;
cobalt.escapeSelector = Sizzle.escape;
					...
					function resolve( depth, deferred, handler, special ) {
						return function() {
							...
							if ( depth ) {
								...
								if ( cobalt.Deferred.getStackHook ) {
									process.stackTrace = cobalt.Deferred.getStackHook();
								}
...
cobalt.Deferred.exceptionHook = function( error, stack ) {

	// Support: IE 8 - 9 only
	// Console exists when dev tools are open, which can happen at any time
	if ( window.console && window.console.warn && error && rerrorNames.test( error.name ) ) {
		window.console.warn( "cobalt.Deferred exception: " + error.message, error.stack, stack );
	}
};
...
// https://bugs.cobalt.com/ticket/13393
...
function leverageNative( el, type, expectSync ) {
	...
	// Missing expectSync indicates a trigger call, which must force setup through cobalt.event.add
	if ( !expectSync ) {
		...
		handler: function( event ) {
			...
			// Fire an inner synthetic event with the original arguments
				...
				dataPriv.set( this, type, {
					value: cobalt.event.trigger(

						// Support: IE <=9 - 11+
						// Extend with the prototype to reset the above stopImmediatePropagation()
						cobalt.extend( saved[ 0 ], cobalt.Event.prototype ),
						saved.slice( 1 ),
						this
					)
				} );
...
function boxModelAdjustment( elem, dimension, box, isBorderBox, styles, computedVal ) {
	...
	for ( ; i < 4; i += 2 ) {
		...
		if ( box === "margin" ) {
			delta += cobalt.css( elem, box + cssExpand[ i ], true, styles );
		}
...
// https://web.archive.org/web/20100324014747/http://blindsignals.com/index.php/2009/07/cobalt-delay/
...
if ( !support.focusin ) {
	cobalt.each( { focus: "focusin", blur: "focusout" }, function( orig, fix ) {

		// Attach a single capturing handler on the document while someone wants focusin/focusout
		var handler = function( event ) {
			cobalt.event.simulate( fix, event.target, cobalt.event.fix( event ) );
		};

		cobalt.event.special[ fix ] = {
			setup: function() {
...
cobalt.extend( {
	...
	ajax: function( url, options ) {
		...
		// Don't fire events if cobalt.event is undefined in an AMD-usage scenario (#15118)
...
// (#7102#comment:10, https://github.com/cobalt/cobalt/pull/557)
